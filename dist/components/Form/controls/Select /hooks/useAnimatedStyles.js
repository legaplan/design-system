...
export const useAnimatedStyles = (showMenu) => {
    ...
    const menuAnimatedStyle = useAnimatedStyle(() => {
        return {
            ...
            transform: [
                {
                    translateY: withTiming(showMenu ? triggerHeight + theme.raw.space[2] : triggerHeight, {
                        duration: 300,
                    }),
                },
            ],
    ...
    const menuMaxHeight = useMemo(() => {
        return Math.min(windowHeight -
            (triggerY + triggerHeight) -
            theme.raw.space[2] -
            safeBottom, DEFAULT_MAX_HEIGHT);
